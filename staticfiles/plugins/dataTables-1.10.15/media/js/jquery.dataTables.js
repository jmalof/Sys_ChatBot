...
(function( $, window, document, undefined ) {
	...
	var _pluck = function ( a, prop, prop2 ) {
		...
		// Could have the prueba in the loop for slightly smaller code, but speed
		// is essential here
		if ( prop2 !== undefined ) {
			for ( ; i<ien ; i++ ) {
				if ( a[i] && a[i][ prop ] ) {
	...
	function _fnGetObjectDataFn( mSource )
	{
			      ...
			      mSource.indexOf('[') !== -1 || mSource.indexOf('(') !== -1) )
		{
			 ...
			 * level down. On each loop we prueba for undefined, and if found immediately
	...
	function _fnSort ( oSettings )
	{
		...
		if ( _fnDataSource( oSettings ) != 'ssp' && aSort.length !== 0 )
		{
			 ...
			 *  }
			 * Basically we have a prueba for each sorting column, if the data in that column is equal,
			 * prueba the next column. If all columns match, then we use a numeric sort on the row
	 ...
	 *      selector for the table to prueba. Note that if more than more than one
	...
	DataTable.defaults = {
		 ...
		 *    } );
		 */
		"aLengthMenu": [[1, 10, 25, 50, 100, -1], [1, 10, 25, 50, 100, "All"]],
